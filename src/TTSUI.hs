...
repair :: Double -> Double -> Double
repair reduction x
   | x >= minPercent * 2 = x - reduction
   | x < minPercent = minPercent
   | otherwise = x
...
computeWidths vals = widths where
    ...
    shortfalls = filter (< minPercent) widths
    shortFallAmount = sum $ map (0.15 -)  shortfalls
    sufficient = fromIntegral (length (filter (>= (minPercent * 1.5)) widths))
    reduction = if sufficient > 0 then shortFallAmount / sufficient else 0
    finalWidths = if sufficient > 0 then map (repair reduction) widths else widths
...
|] where
    ...
    headHeight = numToT (inferRowHeight tableWidth header)
